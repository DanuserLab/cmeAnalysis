function pm = dasParameter(varargin)
ip = inputParser;
ip.CaseSensitive = false;
ip.addParameter('data_all', [], @iscell);
ip.addParameter('con_name', [], @iscell);
ip.addParameter('dir_alt', [], @ischar);
...
ip.addParameter('dist_perct',-1, @isnumeric);
ip.addParameter('fig_disp_mod','default',@ischar); %disable group figure: 'single' 
...
pm = struct(...
    ...
    'n_bar',5,...
    'fig_disp_mod',ip.Results.fig_disp_mod,...
    'col_map',[],...
    'dist_perct',ip.Results.dist_perct,...
 ...
 %==================================================================
 pm.data_all = data_all;
 pm.con_name = con_name;
 pm.dir_alt = dir_alt;

%===================================================================
pm.color_clus = cell(pm.num_clus,1); 
pm.color_clus{1} = [0 0 1]; 
pm.color_clus{2} = [255/255 109/255 182/255]; 
...
col_cond_tem =[0         0         0;
    ...
    0.8588    0.8196         0;
         0    0.5725    0.5725;
    0.2863         0    0.5725;
    0.0028    0.6014    0.3383;
    0.0774    0.4331    0.2327;
    0.6701    0.6651    0.7778;
    0.7426    0.9588    0.2343;
    0.7665    0.8500    0.6920;
    0.8683    0.4602    0.3220;
    0.9234    0.5698    0.4442;
    0.5153    0.3050    0.3566];
if num_condition <= max(size(col_cond_tem))
    pm.col_cond(1:num_condition,:) = col_cond_tem(1:num_condition,:);
else
    pm.col_cond(1:max(size(col_cond_tem)),:) = col_cond_tem(1:max(size(col_cond_tem)),:);
    pm.col_cond(max(size(col_cond_tem))+1:num_condition,:) = rand(num_condition-max(size(col_cond_tem)),3);
end
...
space_tem = (0:0.01:1);
figure;pm.col_map = colormap('jet'); close(gcf);
%pm.col_map = [pm.col_map(1,:)+(-pm.col_map(1,:)).*flipud(space_tem');pm.col_map;pm.col_map(end,:)+(1-pm.col_map(end,:)).*space_tem'];
         
    
    
save([dir_alt filesep 'pm.mat'],'pm')
